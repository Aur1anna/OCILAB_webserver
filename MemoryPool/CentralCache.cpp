...
// 小于等于32KB的请求，使用的每次从PageCache获取span(跨度)大小（以页为单位）
static const size_t SPAN_PAGES = 8;

/*
test_and_set：  这是atomic_flag的原子操作，尝试将标志设置为true，如果之前已经为true，则返回false，否则将其设置为true并返回true。
                配合while：尝试获取锁，如果锁已经被其他线程获取，则一直等待直到获取成功。
memory_order_acquire
                保证后续的内存操作不会被重排序到该操作之前
this_thread::yield()：
                让线程在等待时主动让出CPU，这样其他线程有机会运行，减少了竞争情况下的CPU占用。
*/
