...
void* ThreadCache::allocate(size_t size)
{
    ...
    if (void* ptr = freeList_[index])
    {
        freeList_[index] = *reinterpret_cast<void**>(ptr); // 将freeList_[index]指向的内存块的下一个内存块地址（取决于内存块的实现）
                //指针解引用 *reinterpret_cast<void**>(ptr) 将 ptr 强制转换为指向指针的指针（void**），然后解引用获取该地址存储的值。
                //这实际上是读取内存块头部保存的"下一个空闲块地址"。
        return ptr;
    }
...
// 判断是否需要将内存回收给中心缓存，维护最大freeListSize_为freeListSize_MAX=64
bool ThreadCache::shouldReturnToCentralCache(size_t index)
{
    // 设定阈值，例如：当自由链表的大小超过一定数量时
    size_t freeListSize_MAX = 64; // 例如，64个内存块
    return (freeListSize_[index] > freeListSize_MAX);
}
...
// 由于维护最大freeListSize_为freeListSize_MAX=64，所以超过的要返回给中心缓存
            // 传入size是因为要确认在处理哪个index的freelist时触发的回收，相当于传入index。
void ThreadCache::returnToCentralCache(void* start, size_t size)
{
    ...
    for (size_t i = 0; i < keepNum - 1; ++i) 
    {
        ...
        {
            // 如果链表提前结束，更新实际的返回数量
            // 为什么多余返回时会链表提前结束，不是按照保留四分之一的逻辑来找断点吗？
            //代码中的这个检查本质上是一种fail-safe机制，确保维护内存池的稳定性（宁可不归还，也不能崩溃）
